...
const HeartRateFrequencyScreen = ({navigation}) => {
  ...
  const mutation = useMutation({
    mutationFn: async seconds => {
      return fetcher({
        ...
        data: {data: `[hrtstart,${seconds}]`},
      });
    },
    onSuccess: () => {
      Alert.alert('Success', 'Heart rate frequency updated successfully');
      refetch();
    },
    onError: error => {
      console.error('Error updating Heart rate frequency:', error);
      Alert.alert('Error', 'Failed to update Heart rate frequency');
    },
  });
  ...
  const handleSave = () => {
    ...
    if (!selectedOption) {
      Alert.alert('Error', 'Please select a Heart rate frequency.');
      return;
    }
...
export default HeartRateFrequencyScreen;
